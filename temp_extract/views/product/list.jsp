                            ...
                            <c:forEach var="product" items="${products}">
                                <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 group">
                                    ...
                                    <div class="relative aspect-square overflow-hidden">
                                        <c:choose>
                                            <c:when test="${product.images != null && not empty product.images}">
                                                <img src="${pageContext.request.contextPath}${product.images[0].imageUrl}" 
                                                     alt="${product.name}"
                                                     class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300">
                                            </c:when>
                                            <c:otherwise>
                                                <img src="${pageContext.request.contextPath}/images/products/balo.png" 
                                                     alt="${product.name}"
                                                     class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300">
                                            </c:otherwise>
                                        </c:choose>
                                        ...
                                        <c:url var="prevUrl" value="">
                                            <c:forEach var="param" items="${paramValues}">
                                                <c:if test="${param.key != 'page'}">
                                                    <c:param name="${param.key}" value="${param.value[0]}"/>
                                                </c:if>
                                            </c:forEach>
